  ...
  return (
    <div className="space-y-4">
      <section className="rounded-2xl border border-emerald-100 bg-gradient-to-br from-emerald-50 to-white p-5 shadow-sm">
        <div className="h-4 w-48 animate-pulse rounded bg-emerald-100" />
        <div className="mt-3 space-y-2">
          <div className="h-3 w-full animate-pulse rounded bg-emerald-100/80" />
          <div className="h-3 w-4/5 animate-pulse rounded bg-emerald-100/70" />
        </div>
        <div className="mt-4 grid grid-cols-2 lg:grid-cols-4 gap-2">
          {[0, 1, 2, 3].map((item) => (
            <div key={item} className="rounded-lg border border-emerald-100 bg-emerald-50 px-3 py-3">
              <div className="h-2 w-16 animate-pulse rounded bg-emerald-100" />
              <div className="mt-2 h-3 w-24 animate-pulse rounded bg-emerald-100" />
            </div>
          ))}
        </div>
      </section>
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
        {[0, 1].map((card) => (
          <section key={card} className="rounded-xl border border-border bg-white p-4 shadow-sm space-y-3">
...
export default function LeadsIntelligenceTab({ token, leadId, lead }) {
  ...
  if (!lead) {
    return <div className="rounded-md border border-border bg-white shadow-sm p-5 text-sm text-text-muted">Choose a lead to view AI intelligence.</div>;
  }
  ...
  if (!intelligence) {
    return <div className="rounded-md border border-border bg-white shadow-sm p-5 text-sm text-text-muted">No AI insights are available for this lead yet.</div>;
  }
